...
import { getTodayQuiz, submitVote, getVoteStats, getIsVoteState, getMyInfo } from './api';
...
function App() {
  ...
  const fetchMyInfo = useCallback(async ({ showSuccessAlert = false } = {}) => {
    try {
      const userData = await getMyInfo();
      setUser(userData);

      if (showSuccessAlert) {
        const displayName = userData?.nickname || userData?.name || userData?.email || '사용자';
        showAlert('success', `${displayName}님, 로그인되었습니다.`);
      }
      ...
      setUser(null);
      if (showSuccessAlert) {
        showAlert('error', '로그인 상태 확인에 실패했습니다. 다시 시도해 주세요.');
      }
      return null;
    } finally {
      setAuthChecked(true);
  ...
  const handleLoginSuccess = useCallback(async () => {
    await fetchMyInfo({ showSuccessAlert: true });
  }, [fetchMyInfo]);
        ...
        <div className="stock-info">
          ...
          <div className="price-row">
            {/* <div className="price-item">
              <span className="price-label">전일 종가</span>
              <span className="price-value">{quiz.basePrice?.toLocaleString()}원</span>
            </div>
